...
To illustrate: `639` can be read as `six hundred thirty nine` and written as `6 &#215 100 + 3 &#215 10 + 9 &#215 1`.

<iframe src="https://drive.google.com/file/d/1wQJF5uCpg93kjkDVH51IpT2DhfFEuK0l/preview" width="640" height="480"></iframe>
...
In base 3, the numeral symbols we can work with are `0`, `1`, and `2`. This is like in base 10, where the symbols we use are `0` to `9`.

<iframe src="https://drive.google.com/file/d/1w2EeUub6rt-u5WDRjpWye00Vx5J4A3IM/preview" width="640" height="480"></iframe>

The number above, `201` in base 3, is equal to `19` in base 10. Why? Because `201` (in base 3) is `2 &#215 9 + 0 &#215 3 + 1 &#215 1` which equals `19` (base 10).
...
Here, `2 &#215 27` ( = `54`) is the largest multiple of `27` that can fit into `65`. Let's put a `2` in that slot. Our remainder (what we have left of `65` to work with) is `65 - 54` = `11`.

<iframe src="https://drive.google.com/file/d/1Hl1RKm1gNt3SI5FcJQM-SJEhEcA-DiMN/preview" width="640" height="480"></iframe>

Next is the `nines` position. How many `nines` can fit into our remainder, `11`? `9 &#215 2`? No, that's too big. `9 &#215 1`? Yes! Let's put a `1` in the `nines` position and calculate our remainder (`11 - 9` = `2`).

<iframe src="https://drive.google.com/file/d/1pT7C5UUiEU5tWDPS2Zerx7AEDmRsyv1-/preview" width="640" height="480"></iframe>

Let's continue to the `threes` position. How many `threes` can fit into `2`? None. We put a `0` in the `threes` position and calculate our remainder (`3 - 0` = `3`). Let's try the `ones` position. `2 &#215 1` is 2, so we put a `2` in the `ones` position and calculate our remainder.
...
So, the hexademical number `2A5C` has the value `2 &#215 4096 + 10 &#215 256 + 5 &#215 16 + 12 &#215 1` which is equal to `10844` in base 10. 
